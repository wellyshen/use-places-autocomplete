...
- 🦔 Tiny size ([~ 1.5KB gzipped](https://bundlephobia.com/result?p=use-places-autocomplete)). No external dependencies, aside for the `react`.
...
const methods = usePlacesAutocomplete({
  // Provide the cache time in milliseconds, default is 24 hours
  cache: 24 * 60 * 60 * 1000,
});
```

> The response data is cached via the [Window.sessionStorage API](https://developer.mozilla.org/en-US/docs/Web/API/Window/sessionStorage).
...
| `cache`          | number \| false | `86400000` (24 hours) | Number of milliseconds to [cache the response data of Google Maps Places API](#cache-data-for-you).                                                                                                                     |
